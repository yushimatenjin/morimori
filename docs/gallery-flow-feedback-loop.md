...
## 横浜表示不全 ループ記録（2026-03-15）

- 症状:
  - 横浜を含む生成で「表示されない / 極端に小さい」体感が発生
- 再現条件:
  - 2地点モード（例: 富士山→横浜）で自動選択された高Zoom（14付近）
  - DEM欠損と無効標高値が多発し、地形のバウンディングが破綻
- 原因:
  - DEM無効値 `8388608` を通常値として扱い、極端な負標高として解釈していた
  - 2地点モードの自動Zoom上限が高く、GSIの欠損タイル帯に入りやすかった
- 実装対応:
  - `src/services/map-data-provider.js`
    - DEM無効値 `8388608` を 0m 扱いへ修正
    - 標高値を `-200m〜4500m` にクランプ
    - `invalidHeightCount` を診断値として返却
    - DEM欠損時は写真タイルから疑似標高を生成して補完
  - `src/App.tsx`
    - 補完警告に `無効標高` 件数を表示
    - 2地点モードの自動Zoom候補を `12以下` に制限
- 検証結果:
  - Playwright実操作で `generated` 到達を確認
  - `ViewerRoot` に canvas 描画あり
  - ステータスに補完警告を表示し、無言失敗を排除
- 判定:
  - 継続（ship可）: 「表示ゼロ」は再現せず、失敗時の可視化と復帰導線を確保
